...
output_folder = 'code/full_backtest'
os.makedirs(output_folder, exist_ok=True)

# Load data
df_binance = pd.read_csv('binance_futures_btc_usdt_1h_2021_2023.csv')
df_binance['Timestamp'] = pd.to_datetime(df_binance['Timestamp'])
df_binance.set_index('Timestamp', inplace=True)
df_okx = pd.read_csv('okx_futures_btc_usdt_1h_2021_2023.csv')
df_okx['Timestamp'] = pd.to_datetime(df_okx['Timestamp'])
df_okx.set_index('Timestamp', inplace=True)
df_btc_spot = pd.read_csv('binance_spot_btc_usdt_1h_2021_2023.csv')
...
# Assuming df_binance and df_okx are pandas DataFrames with datetime index
...
# Optional: Verify the split (you can add this for debugging)
